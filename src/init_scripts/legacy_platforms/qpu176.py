...
PLATFORM_NAME = "qpu176"        # Used for the data directory and device config file name.
LOAD_CFG_FILE = False           # Set True to load hardware config from the saved JSON file.
from init_scripts.hw_configs.cfg_qpu176 import HW_CONFIG_DICT
...
def initialize(
    ...
    # -- Qubit elements --
    helper_configure_ladder(quantum_device, num_qubits=8, feedlines={"f0": ["q0", "q1"]})

    # -- Initial values for qubit parameters, these should be loaded from snapshots after calibration
    if load_defaults:
        helper_defaults(
            quantum_device,
            clocks=[4.7869e9, 5.0192e9],
            readouts=[7.4044e9, 7.6561e9],
        )
