"""
Initialization script for TII QPU182.

Author: Tatiana Kazieva, TII QRC
Version: 1.0
Date: 2026-07-07 (YYYY/DD/MM)

This script sets up the hardware configuration, instrument connections, and quantum
device representation for the TII QPU182. Platform-specific constants are defined at
the top; shared boilerplate is delegated to :mod:`init_scripts._common`.
"""

CLUSTER_IP    = "192.168.0.20"  # IP address of the cluster.
PLATFORM_NAME = "qpu182"        # Used for the data directory and device config file name.
LOAD_CFG_FILE = False           # Set True to load hardware config from the saved JSON file.
from init_scripts.hw_configs.cfg_qpu182 import HW_CONFIG_DICT
...
def initialize(
    ...
    """
    Initialize QPU182 and return the configured QuantumDevice.
    ...
    if load_defaults:
        helper_defaults(
            quantum_device,
            clocks=[3.88e9, 4.02e9, 4.229e9, 4.342, 3.88e9, 4.02e9, 4.229e9, 4.342],
            readouts=[7.01e9, 7.09e9, 7.19e9, 7.28e9, 7.01e9, 7.09e9, 7.19e9, 7.28e9],
        )
