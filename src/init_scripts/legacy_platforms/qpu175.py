...
def initialize(
    ...
    """
    Initialize QPU175 and return the configured QuantumDevice.
    ...
    # -- Qubit elements --
    helper_configure_ladder(quantum_device, num_qubits=4, feedlines={"f0": ["q0", "q1", "q2", "q3"]})

    # -- Initial values for qubit parameters, these should be loaded from snapshots after calibration
    if load_defaults:
        helper_defaults(
            quantum_device,
            clocks=[3.88e9, 4.02e9, 4.22e9, 4.34e9],
            readouts=[7.39e9, 7.62e9, 7.75e9, 7.97e9],
        )
...
qubits = [quantum_device.get_element(f"q{i}") for i in range(4)]
q0, q1, q2, q3 = qubits
