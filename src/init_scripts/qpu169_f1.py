...
PLATFORM_NAME = "qpu169_f1"        # Used for the data directory and device config file name.
LOAD_CFG_FILE = False           # Set True to load hardware config from the saved JSON file.
from init_scripts.hw_configs.cfg_qpu169_f1 import HW_CONFIG_DICT
...
def initialize(
    ...
    # -- Qubit elements --
    helper_configure_ladder(quantum_device, num_qubits=8, feedlines={"f1": ["q4", "q5", "q6","q7"]})

    # -- Initial values for qubit parameters, these should be loaded from snapshots after calibration
    if load_defaults:
        helper_defaults(
            quantum_device,
            clocks=[3.88e9, 4.02e9, 4.229e9, 4.342],
            readouts=[7.01e9, 7.09e9, 7.19e9, 7.28e9],
        )
...
qubits = [quantum_device.get_element(f"q{i+4}") for i in range(4)]
q4, q5, q6, q7 = qubits
feedlines = [quantum_device.get_element(f"f{i+1}") for i in range(1)]
f1 = feedlines
